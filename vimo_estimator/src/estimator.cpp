...
void Estimator::processIMUandThrust(double dt, const Vector3d &linear_acceleration, const Vector3d &angular_velocity, double Fz)
{   
    if (!first_imu)
    {
        first_imu = true;
    ...
    // TODO: Test switching to VIO from VIMO when estimator receives zero control input and vice versa
    // if (USE_VIMO)
    // {
    //     if (Fz <= 1e-3)
    //     {
    //         ROS_WARN("Receiving ZERO control input, switching OFF VIMO, switching ON VIO");
    //         use_vimo = false;
    //     }
    //     else
    //     {
...
void Estimator::optimization()
{
    ...
    for (int i = 0; i < WINDOW_SIZE; i++)
    {
        ...
        if(use_vimo)
        {
            ...
            problem.AddResidualBlock(model_factor, NULL, para_Position[i], para_Attitude[i], para_Speed[i], para_Fext[i], para_Position[j], para_Speed[j]);  
        }         
    }
    int f_m_cnt = 0;
    int feature_index = -1;
